<template>
  <div class="log">
        ...
        <div v-permission="'搜索日志'" class="header-right">
          ...
          >
            <el-button size="medium">
              {{ searchUser ? searchUser : '全部人员' }} <i class="el-icon-arrow-down el-icon--right" />
            </el-button>
          ...
          <lin-date-picker ref="searchDate" class="date" @dateChange="handleDateChange" />
        </div>
      </div>
    ...
    <div v-loading="loading" class="content">
      <article>
        <section v-for="item in logs" :key="item.id">
          <span class="point-time" />
          <aside>
            <p class="things" v-html="item.message" />
            <p class="brief">
              <span class="text-yellow">{{ item.username }}</span> {{ item.time | dateTimeFormatter }}
            </p>
          </aside>
        </section>
      </article>
      ...
      <div v-if="logs && logs.length">
        ...
        <div class="more" :class="{ nothing: finished }">
          <i v-if="more" class="iconfont icon-loading" />
          <div v-show="!more && !finished" @click="nextPage">
            <span>查看更多</span> <i class="iconfont icon-gengduo" style="font-size:14px" />
          </div>
          <div v-if="finished">
      ...
      <div v-else class="nothing">暂无日志信息</div>
    </div>
  </div>
</template>
