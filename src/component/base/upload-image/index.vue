...
<template>
  <div v-loading="loading" class="upload-imgs-container">
    <template v-for="(item, i) in itemList">
      <template v-if="item.display">
        <div :key="item.id" v-loading="item.loading" class="thumb-item" :style="boxStyle">
          <el-image class="thumb-item-img" :src="item.display" :fit="fit" style="width: 100%; height: 100%;" />
          <div class="info">
            <i
              v-if="item.file"
          ...
          <div class="control">
            <i v-if="!disabled" class="el-icon-close del" title="删除" @click.prevent.stop="delItem(item.id)" />
            <div v-if="!disabled" class="preview" title="更换图片" @click.prevent.stop="handleClick(item.id)">
              <i class="el-icon-edit" />
            </div>
            <div v-if="sortable || preview" class="control-bottom">
              <i
      ...
      <template v-else>
        ...
        >
          <i class="el-icon-plus" style="font-size: 3em;" />
          <div style="margin-top: 1em;" v-html="rulesTip.join('<br>')" />
        </div>
      </template>
    </template>
    <input
