<template>
  <div v-if="histories.length > 1" ref="resueTab" class="reuse-tab">
    <swiper :options="swiperOption" class="reuse-tab-wrap">
      <swiper-slide v-for="(item, index1) in histories" :key="item.path">
        <router-link
          class="reuse-tab-item"
          :class="item.path === $route.path ? 'active' : ''"
          :to="item.path"
          @contextmenu.prevent.native="onTags(index1, $event)"
        >
          <i v-if="!filterIcon(stageList[item.stageId].icon)" :class="stageList[item.stageId].icon" />
          <img v-else :src="stageList[item.stageId].icon" style="width:16px;">
          <span style="padding: 0 5px;">{{ stageList[item.stageId].title }}</span>
          <span class="el-icon-close" @click.prevent.stop="close(index1)" />
        </router-link>
      </swiper-slide>
    </swiper>
