<template>
  <div class="app-sidebar">
    <div v-if="!elMenuCollapse" class="logo"><img src="../../assets/image/logo.png" alt=""></div>
    <div v-else class="mobile-logo"><img src="../../assets/image/mobile-logo.png" alt=""></div>
    <div style="margin-bottom:50px">
      <div v-if="showSidebarSearch" style="margin-top: 15px">
        <div v-if="!showSearchList" class="search-display" @click="toSearch"><i class="el-icon-search" /></div>
        <el-select
          v-if="showSearchList"
          ref="searchInput"
          v-model="sidebar"
      ...
      >
        <template v-for="item in sideBarList">
          ...
          >
            <template slot="title">
              <i v-if="!filterIcon(item.icon)" :class="item.icon" /> <img v-else :src="item.icon" class="imgIcon">
            ...
            <template v-for="subItem in item.children">
              ...
              >
                <template slot="title">
                  <i class="iconfont icon-erjizhibiao" />
              ...
              <!-- 二级else -->
              <router-link v-else :key="subItem.name" :to="subItem.path" class="circle">
          ...
          <router-link v-else :key="item.name" :to="item.path">
            <el-menu-item :key="idMap[item.name]" class="subMenuContent" :index="idMap[item.name]">
              <i v-if="!filterIcon(item.icon)" :class="item.icon" /> <img v-else :src="item.icon" class="imgIcon">
              <span slot="title">{{ item.title }}</span>
            </el-menu-item>
          </router-link>
